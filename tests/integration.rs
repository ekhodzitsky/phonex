...
fn test_transcribe_0_wav() {
    ...
    assert!(
        text.contains("เกม"),
        "expected 'เกม' in transcription, got: {}",
        text
    );
    assert!(
        text.contains("อินโดนีเซีย"),
        "expected 'อินโดนีเซีย' in transcription, got: {}",
        text
    );
}
...
fn test_transcribe_1_wav() {
    ...
    assert!(!text.is_empty(), "transcription should not be empty");
    assert!(
        text.contains("แข่งขัน"),
        "expected 'แข่งขัน' in transcription, got: {}",
        text
    );
}
...
fn test_transcribe_batch() {
    ...
    let (samples0, sr0) =
        phonex::audio::AudioPreprocessor::read_wav(&format!("{}/test_wavs/0.wav", MODEL_DIR))
            .unwrap();
    let (samples1, sr1) =
        phonex::audio::AudioPreprocessor::read_wav(&format!("{}/test_wavs/1.wav", MODEL_DIR))
            .unwrap();
    ...
    let results = engine
        .transcribe_batch(vec![&samples0, &samples1], &mut guard)
        .expect("batch transcription failed");

    assert_eq!(results.len(), 2);
    assert!(
        !results[0].text.is_empty(),
        "first transcription should not be empty"
    );
    assert!(
        !results[1].text.is_empty(),
        "second transcription should not be empty"
    );
    assert!(
        results[0].text.contains("เกม"),
        "expected 'เกม' in first, got: {}",
        results[0].text
    );
    assert!(
        results[1].text.contains("แข่งขัน"),
        "expected 'แข่งขัน' in second, got: {}",
        results[1].text
    );
}
...
fn test_transcribe_vad_2_wav() {
    ...
    let mut guard = engine.pool.try_checkout().expect("pool empty");
    let result = engine
        .transcribe_samples_with_vad(&samples, &mut guard)
...
fn test_streaming_en_pipeline() {
    ...
    let (samples, sample_rate) = phonex::audio::AudioPreprocessor::read_wav(&format!(
        "{}/test_wavs/0.wav",
        STREAMING_EN_DIR
    ))
    .unwrap();
    ...
    while offset + chunk_samples < samples.len() {
        let _tokens = pipeline
            .accept_audio(&samples[offset..offset + chunk_samples])
            .unwrap();
...
fn streaming_transcribe(
    ...
    while offset + chunk_size < samples.len() {
        let _tokens = pipeline
            .accept_audio(&samples[offset..offset + chunk_size])
            .unwrap();
        offset += chunk_size;
    }
    if offset < samples.len() {
...
fn test_streaming_en_1_wav() {
    ...
    assert!(
        !text.is_empty(),
        "streaming transcription of 1.wav should not be empty"
    );
}
...
fn test_streaming_en_8k_wav() {
    ...
    let samples = phonex::audio::AudioPreprocessor::typhoon()
        .resample(&samples, sample_rate)
        .unwrap();
    ...
    assert!(
        !text.is_empty(),
        "streaming transcription of 8k.wav should not be empty"
    );
}
...
fn test_streaming_different_chunk_sizes() {
    ...
    for chunk_size in [1000, 4000, 8000, 16000] {
        ...
        assert!(
            !text.is_empty(),
            "streaming transcription with chunk_size={} should not be empty",
            chunk_size
        );
    }
}
...
fn test_streaming_reset_reuse() {
    ...
    let (samples0, sr0) = phonex::audio::AudioPreprocessor::read_wav(&format!(
        "{}/test_wavs/0.wav",
        STREAMING_EN_DIR
    ))
    .unwrap();
    ...
    assert!(
        !text0.is_empty(),
        "first streaming transcription should not be empty"
    );

    pipeline.reset();

    let (samples1, sr1) = phonex::audio::AudioPreprocessor::read_wav(&format!(
        "{}/test_wavs/1.wav",
        STREAMING_EN_DIR
    ))
    .unwrap();
    ...
    assert!(
        !text1.is_empty(),
        "second streaming transcription should not be empty"
    );
    assert_ne!(
        text0, text1,
        "reset and reuse should produce different transcription for different audio"
    );
}
